...
DEFAULT_LD_LABEL = "com.simple.deferred-update-policy"
# Trigger: What custom trigger should be called to actually kick off the policy?
DEFAULT_LD_JAMF_TRIGGER = "deferred_update_policy"
...
# Prompt GUI Config
GUI_WINDOW_TITLE = "Simple IT Notification"
GUI_HEADING = "macOS Updates Available"
GUI_ICON = ("/private/var/tmp/logo-rings.png")
GUI_MESSAGE = """Sorry for the pop up(we'll try and avoid these in the future), but we are required by BBVA to deploy a one-time update for your Mac to get your computer super safe and secure.

Some required updates will require rebooting your computer once installed, and could take up to 30-45 minutes. PLEASE make sure your computer is plugged into power.

Because we love you, you may schedule these updates for a convenient time by choosing when to start installation, but please install by the end of the day.
...
GUI_DEFER_OPTIONS = ["300", "0", "1800", "3600", "7200", "14400", "28800"]
GUI_BUTTON = "Okay"

# Confirmation dialog Config
GUI_S_HEADING = "Update scheduled"
GUI_S_ICON = ("/private/var/tmp/logo-rings.png")
GUI_S_BUTTON = "OK"
...
GUI_S_MESSAGE = """Great! Installation of required updates will begin on {date}. We'll prompt you again after they're installed(~10-15 minutes). Don't forget to save your work and backup anything important!"""
...
GUI_E_MESSAGE = ("A problem occurred processing your request. Please contact "
                 "IT via Freshdesk for assistance.")
...
def calculate_deferment(add_seconds):
    ...
    future = now + diff
    return (int(future.strftime("%m")),
            int(future.strftime("%d")),
...
def main():
    ...
    if args.mode == 'prompt':
        ...
        if secs == 0:
            ...
            month, day, hour, minute, datestring = calculate_deferment(secs)
            daemon['StartCalendarInterval'] = {'Month': month,
                                            'Day': day,
                                            'Hour': hour,
                                            'Minute': minute
                                            }
